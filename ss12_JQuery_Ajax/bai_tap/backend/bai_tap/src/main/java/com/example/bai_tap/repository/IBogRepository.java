...
public interface IBogRepository extends JpaRepository<Blog, Integer> {
    ...
    List<Blog> findByisDeleteIsFalse();
    ...
    List<Blog> findByBlogType_idBlogType(int id);

    List<Blog> searchBlogByTileBlogContainingIgnoreCase(String tileBlog);

    @Query(value = "select * from blog limit :num ", nativeQuery = true)
    List<Blog> getBlogLimit(@Param("num") int num);
